  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl md:text-2xl font-bold text-gray-900">Inspections</h1>
            ...
            <div className="divide-y divide-gray-50">
              {inspections.map((insp) => (
                <Link key={insp.id} href={`/inspections/${insp.id}`}
                  className="block hover:bg-amber-50/40 transition-colors"
                >
                  {/* Mobile card layout */}
                  <div className="md:hidden px-4 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="font-medium text-sm text-gray-900 mt-0.5">
                          {insp.customer ? `${insp.customer.firstName} ${insp.customer.lastName}` : "—"}
                        </div>
                        <div className="text-xs text-gray-500 mt-0.5 truncate">{insp.propertyAddress}</div>
                        <div className="text-xs text-gray-400 mt-1">
                          {chimneyTypeLabel(insp.chimneyType)} · {inspectionLevelLabel(insp.inspectionLevel)} · {formatDate(insp.inspectionDate)}
                        </div>
                      </div>
                  ...
                  <div className="hidden md:grid grid-cols-12 px-6 py-4 items-center">
                    ...
                    <div className="col-span-2">
                      {insp.overallCondition
                        ? <span className={`text-xs px-2 py-1 rounded-full font-medium ${statusBadgeClass(insp.overallCondition)}`}>{overallConditionLabel(insp.overallCondition)}</span>
